...
void get (){
    ...
    if(fp==NULL){
        ...
        bzero(lineBuffer,1024);
        sprintf(lineBuffer, "%d", file_size);

        if (write(STDOUT_FILENO, lineBuffer, 1024) < 0)
            printf("Couldn't send message\n");
        bzero(lineBuffer,1024);
        int block_size;
        while(strstr(lineBuffer, "rdy")==NULL){
            if(read(STDIN_FILENO, lineBuffer, sizeof(lineBuffer))<0)
                printf("Client: Can not read rdy.\n");
        }
        while ((block_size = fread(lineBuffer, sizeof(char), 1024, fp)) > 0) {

            if (send(STDOUT_FILENO, lineBuffer, block_size, 0) < 0) {
...
void put(){
    ...
    int file_size=atoi(lineBuffer);
    bzero(lineBuffer,1024);


    strcpy(lineBuffer,"rdy");
    if (write(STDOUT_FILENO, lineBuffer, 1024) < 0)
        printf("Couldn't send message\n");
    bzero(lineBuffer,1024);

    while((block_size = recv(STDIN_FILENO, lineBuffer, 1024, 0))>0){

        file_size=file_size-block_size;

        if(block_size<0)
        {
            printf("Server: Daten konnten nicht erhalten werden.\n");
        }

        int write_size = fwrite(lineBuffer, sizeof(char), block_size, fp);

        if(write_size < block_size)
        {
            printf("Server: File write failed on server.\n");
        }else if(file_size<=0) {
            fclose(fp);
            break;
        }
